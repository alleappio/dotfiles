...
wk.add({
    ...
    { '<leader>ft', '<cmd>Neotree toggle<CR>', desc = 'Help tags', mode = 'n' },
})
...
wk.add({
    { '<leader>t', group = 'Terminal' },
    {
        '<leader>tt',
        function()
            terminal_emulator.toggle_split_terminal()
        end,
    ...
    },
    {
        '<leader>tf',
        function()
            if vim.api.nvim_buf_is_valid(terminal_emulator.current.buf) then
                vim.api.nvim_win_set_buf(0, terminal_emulator.current.buf)
            else
                vim.cmd('term')
                terminal_emulator.current.buf = vim.api.nvim_get_current_buf()
            end
            vim.cmd('startinsert')
        end,
        desc = 'Open terminal fullscreen',
        mode = { 'n', 't' },
    },
